...
cpn_affine_transform (GeglBuffer *buffer,
                      gdouble     min,
                      gdouble     max)
{//printf("compose.c cpn_affine_transform\n");
...
fill_buffer_from_components (GeglBuffer   *temp[MAX_COMPOSE_IMAGES],
                             ...
                             ComposeInput *inputs,
                             gdouble       mask_vals[MAX_COMPOSE_IMAGES])
{//printf("compose.c fill_buffer_from_components\n");
  GeglBufferIterator *gi;
  gint                j;
...
perform_composition (COMPOSE_DSC   curr_compose_dsc,
                     ...
                     ComposeInput *inputs,
                     gint          num_images)
{//printf("compose.c perform_composition\n");
...
create_new_image (const gchar    *filename,
                  ...
                  gint32         *layer_ID,
                  GeglBuffer    **buffer)
{//printf("compose.c create_new_image\n");
  gint32            image_ID;
...
compose_dialog (const gchar *compose_type,
                gint32       drawable_ID)
{//printf("2. compose.c compose_dialog\n");
  GtkWidget    *dialog;
  GtkWidget    *main_vbox;
  GtkWidget    *frame;
  GtkWidget    *vbox;
...
check_response (GtkWidget *dialog,
                gint       response,
                gpointer   data)
{//printf("compose.c check_response\n");
  switch (response)
    {
...
combo_callback (GimpIntComboBox *widget,
                gpointer         data)
{//printf("compose.c combo_callback\n");
  gint id;
  gint n;
...
scale_callback (GtkAdjustment *adj,
                ComposeInput  *input)
{//printf("compose.c scale_callback\n");
...
type_combo_callback (GimpIntComboBox *combo,
                     gpointer         data)
{//printf("compose.c type_combo_callback\n");
